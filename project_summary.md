...
python# For files in subdirectories importing from other subdirectories:
...
Running Scripts
bash# ALWAYS run from APAC root:
cd /Volumes/SSD/APAC  # or your APAC path
python3 algorithm/test_file.py   # ✅ Correct
# NOT: cd algorithm && python3 test_file.py ❌
Key Rules

Always use full paths from APAC root (e.g., database.db_manager, not just db_manager)
System path adjustment for test files: Add sys.path.append('.') when in subdirectories
Run all scripts from APAC root directory to ensure imports resolve correctly
Special case: When db_manager imports from same directory, still use from database.historical_pull
Note typos: system_databse (not system_database), algorithm (not algorithms)
...
ColumnTypeDescriptionExampleidINTEGER PRIMARY KEYAuto-increment ID1, 2, 3...display_nameTEXT NOT NULLUnique identifier"NVDA_sma_crossover_20240604_143022"algorithm_typeTEXT NOT NULLAlgorithm filename"sma_crossover"tickerTEXT NOT NULLStock symbol"NVDA"initial_capitalREAL NOT NULLAllocation amount10000.0statusTEXT NOT NULLRunning/stopped"running"created_atTEXT NOT NULLUTC timestamp"2024-06-04T14:30:22Z"stopped_atTEXTUTC when stoppedNULL or "2024-06-04T16:45:00Z"
Table: transactions
ColumnTypeDescriptionExampleidINTEGER PRIMARY KEYAuto-increment ID1, 2, 3...algorithm_idINTEGER NOT NULLLinks to algorithm1typeTEXT NOT NULLBuy or sell"buy"sharesINTEGER NOT NULLShare count100priceREAL NOT NULLExecution price152.45timestampTEXT NOT NULLUTC timestamp"2024-06-04T14:30:00Z"
Table: system_config
ColumnTypeDescriptionExamplekeyTEXT PRIMARY KEYConfig key"pin"valueTEXT NOT NULLConfig value"2020"
Schema
sql-- Algorithm instances with auto-increment IDs
...
Common Queries
sql-- Get all running algorithms
...
Database Structure Detail
Table: stock_prices
ColumnTypeDescriptionExampleminute_timestampTEXT PRIMARY KEYUTC market minute"2024-01-02T14:30:00Z"AAPLTEXTJSON OHLCV data'{"o":150.25,"h":150.50,"l":150.00,"c":150.45,"v":1000000}'NVDATEXTJSON OHLCV data'{"o":450.00,"h":451.25,"l":449.75,"c":450.50,"v":500000}'[ticker]TEXTDynamic columnsAdded as needed per ticker
Key Points:
...
pythondef get_data_for_algorithm(ticker, requirement_type, **kwargs):
    """
...
Common Queries
sql-- Get latest price for ticker (note: minute_timestamp not timestamp)
...
Execute algorithms every minute during market hours
Manage WebSocket subscriptions based on active algorithms
Execute trades through Alpaca and record transactions
Handle errors without crashing (algorithm failures isolated)
...
python# Each bar in the returned list has this exact structure:
{
...
python# Calculate current position from transactions
current_shares = 0
...
current_shares = sum(tx['shares'] if tx['type'] == 'buy' else -tx['shares'] 
                    for tx in transactions)
Testing Algorithms
python# Mock pattern for testing without full system
import sys
sys.path.append('.')  # Add APAC root to path
...
Creation: Frontend calls /api/algorithms with ticker, type, and capital
Registration: System creates database entry with auto-generated ID
Execution: Orchestrator loads algorithm module from /algorithm/ and calls run() every minute
Data Access: Algorithm fetches guaranteed market-minute data (auto-fetch if missing)
Decision: Algorithm returns tuple: ('buy'/'sell'/'hold', shares)
Trade: Orchestrator executes via Alpaca and records transaction
Display: Frontend polls API and shows updated P&L

Data Flow

Real-time: Alpaca WebSocket → realtime_pull.py → stocks.db
Historical: Algorithm request → db_manager → Auto-fetch if missing → stocks.db
Calculations: Transaction history → card_calculations.py → Frontend display
